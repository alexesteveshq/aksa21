...
class UpdateProductPriceWizard(models.TransientModel):
    ...
    def update_price(self):
        ...
        products =  self.env['stock.quant'].search(
            [('product_id', 'in', purchase_order.mapped('order_line.product_id.id'))]).filtered(
            lambda q: q.quantity > 0).mapped('product_id')
        products.with_context(increase_price=True).update_price_percentage(self.price_increase)
